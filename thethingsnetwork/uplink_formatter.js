...
function decodeUplink(input) {
  ...
  switch (input.fPort) {
    case 2:
      if (Ext == 0x09) {
        var dsRaw = ((bytes[0] << 24) >> 16) | bytes[1];
        if (bytes[0] & 0x80) {
          dsRaw |= 0xffff0000;
        }
        data.TempC_DS = parseFloat((dsRaw / 100).toFixed(2));
      ...
      if (Ext != 0x0f) {
        var shtRaw = ((bytes[2] << 24) >> 16) | bytes[3];
        if (bytes[2] & 0x80) {
          shtRaw |= 0xffff0000;
        }
        data.TempC_SHT = parseFloat((shtRaw / 100).toFixed(2));
        data.Hum_SHT = parseFloat(
      ...
      if (Ext == "0") {
        ...
        var dsRaw = ((bytes[7] << 24) >> 16) | bytes[8];
        if (bytes[7] & 0x80) {
          dsRaw |= 0xffff0000;
        }
        data.TempC_DS = parseFloat((dsRaw / 100).toFixed(2));
